package com.gcs.vppa.common.entity.arraytype;
...
public class StringArrayTypeDescriptor extends AbstractArrayTypeDescriptor<String[]> {
  ...
  private String[] convertToArray(String stringData) {
    ...
    StringArrayTypeDescriptor.ArrayObject arrayList = new StringArrayTypeDescriptor.ArrayObject();
    char[] chars = stringData.toCharArray();
    StringBuilder buffer = null;
    boolean insideString = false;
    boolean wasInsideString = false;
    List<StringArrayTypeDescriptor.ArrayObject> dims = new ArrayList();
    StringArrayTypeDescriptor.ArrayObject curArray = arrayList;
    int startOffset = 0;
      ...
      if (chars[i] == PREFIX_SLASH) {
        ...
        if (!insideString && chars[i] == OPEN_ARRAY) {
          if (dims.isEmpty()) {
            ...
            StringArrayTypeDescriptor.ArrayObject a = new StringArrayTypeDescriptor.ArrayObject();
            StringArrayTypeDescriptor.ArrayObject p = dims.get(dims.size() - 1);
            p.add(a);
            dims.add(a);
          }
        ...
        if (!insideString && (chars[i] == DELIMITER || chars[i] == CLOSE_ARRAY) || i == chars.length - 1) {
          if (chars[i] != DOUBLE_COMMA && chars[i] != CLOSE_ARRAY && chars[i] != DELIMITER && buffer != null) {
